...
- Hero에서 “차량 등록 대기 중”처럼 미완성 서비스로 보이는 문구를 제거하고, 검색/비교/상담 가치 중심 문구로 정리했다.
...
- 데스크톱 카드 그리드는 4열에서 3열로 조정해 이미지와 카드 여백이 더 크게 보이도록 개선했다.
...
- 라우트가 바뀔 때 이전 안내가 계속 남지 않도록 메시지를 정리한다.
- 성공/정보 안내는 4초 뒤 자동으로 닫히도록 처리했다.
