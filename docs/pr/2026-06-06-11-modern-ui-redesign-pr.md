...
- Hero와 추천 매물 섹션에서 “준비 중”처럼 보이는 내부 안내 문구를 제거했다.
...
- 데스크톱 카드 그리드는 3열로 조정해 카드 크기와 이미지 노출 면적을 확보했다.
...
- SPA 이동 후 이전 안내가 남는 문제를 줄이기 위해 라우트 변경 시 메시지를 정리한다.
- 성공/정보 안내는 4초 뒤 자동 닫힘 처리했다.
