...
- [ ] `OPENAI_API_KEY`는 `VITE_*` 환경변수, 프론트엔드 코드, README 실제 값에 작성하지 않는다.
- [ ] `.env.example`에는 실제 OpenAI 키 형식의 값을 넣지 않고 placeholder만 사용한다.
- [ ] 프롬프트 인젝션성 질문(시스템 프롬프트 요청, 이전 지시 무시 요청)은 직접 문의 또는 거절 안내로 처리된다.
- [ ] 같은 AI 상담 요청을 짧은 시간에 반복하면 `429`로 차단된다.
