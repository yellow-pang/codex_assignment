...
3. 로그인 사용자가 사이트 오른쪽 아래 플로팅 AI 챗봇 버튼을 열고 질문한 경우
...
사이트 공통 AI 챗봇은 `/api/chats/site-bot/messages` REST API를 사용한다.
플로팅 버튼은 상담방 화면을 제외한 일반 화면 오른쪽 아래에 표시되며, 비로그인 사용자는 모달 안에서 로그인 안내를 본다.
