...
현재 `.env.example`의 환경변수 이름은 Render Environment에도 반영된 상태이며, 이번 문서 정리에서는 환경변수 이름을 추가하거나 변경하지 않는다.
...
현재 `.github/workflows/deploy.yml`은 위 설명과 일치하므로 이번 문서 정리 단계에서는 workflow 파일을 수정하지 않는다.
